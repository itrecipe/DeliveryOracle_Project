...
- 각자가 맡은 기능이 구현이 되었는지 진행중인지 시작 전인지에 대해<br/>
  진행 상황을 파악하기 위해 작업 진행중인 경우 **"진행중"** 완료가 되었다면<br/>
  **"완료"** 상태 값을 찍어 놓고 표시 해두기로 약속하였습니다.<br/><br/>
  모든 팀원이 팀 회의를 통해 작업 진행 상황을 공유하며 진행 하였으며,<br/>
  작업을 이행해야 하는 날짜에 아무런 상태값이 표시되어 있지 않으면 즉시<br/>
  어떤 문제점이 있는지 어떤 어려움이 있는지를 파악하여 함께 공유하고 의견을<br/>
  나누며 문제 해결 방법을 찾아나가는 방향으로 진행 하였습니다.<br/><br/>
