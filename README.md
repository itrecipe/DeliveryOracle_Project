...
- 각자가 맡은 기능이 구현이 되었는지 진행중인지 시작 전인지에 대한 진행 상황을 파악하기 위해<br/>
  작업을 진행하고 있다면 **"진행중"** 완료가 되었다면 **"완료"** 상태 값을<br/>
  찍어 놓고 표시 해두기로 약속하고 모든 팀원이 함께 작업 진행 상황을 공유하며 진행 하였습니다.<br/><br/>
  기능 구현을 해야하는 일정이 되었음에도 불구하고 아무런 상태값이 보이지 않는 경우,<br/>
  팀 회의를 통해 **현재 작업 진행 상황을 수시로 점검 및 체크** 하였고,<br/>
  자신이 담당하고 있는 파트가 아니어도, 일정에 차질이 없도록 하기 위해<br/>
  현재 자신의 상황을 팀원들과 공유해서 서로 도와가며 개발을 이어가거나,<br/>
  문제해결 방법을 함께 찾아보고 고민하는 시간을 가지며,<br/>
  성실히 작업을 이행 하였습니다.<br/><br/>
